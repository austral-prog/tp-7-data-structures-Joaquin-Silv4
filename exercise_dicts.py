...
def create_inventory(items):
    ...
    for item in items:                     #recorro los items que tengo
        if item in inventario:                          #si hay otro item en los items repetido.Ej: tenes pera en items y otra pera en la mano
            inventario[item]=inventario[item] + 1             #le agrego 1 (ahora tengo 2 peras)
        else:
            inventario[item]= 1                         #si no se repite el item en la lista de item pongo 1
    return inventario                                   #retorno el nuevo inventario
...
def sum_expenses(gastos):
    ...
    for categoria, lista_gastos in gastos.items():
        resultado[categoria]= sum(lista_gastos)
